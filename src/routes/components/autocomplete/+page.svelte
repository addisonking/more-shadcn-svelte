        ...
        <DocPage.Example>
            <DocPage.Preview class="flex flex-col gap-4 min-h-[300px] items-center pt-10">
                <div class="w-full max-w-sm space-y-2">
                    <label for="user-search" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                        Find User
                    </label>
            ...
            <DocPage.Code code={`<Autocomplete
  id="user-search"
  bind:value={query}
  options={filteredUsers}
  {loading}
>
  {#snippet itemSnippet(user)}
    <Highlight text={user.name} {query} />
  {/snippet}
</Autocomplete>`} />
        </DocPage.Example>
